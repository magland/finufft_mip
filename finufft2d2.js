// wasm: finufft
// finufft2d2(x, y, fk_re, fk_im, iflag, tol) -> [c_re, c_im]
// 2D type 2 non-uniform FFT (uniform → nonuniform).
register({
  ...
  apply: function (args, nargout) {
    ...
    var ms = fk_re.shape[0];
    var mt = fk_re.shape[1];
    var nin = ms * mt;
    var BYTES = 8;
    ...
    view.set(new Float64Array(fk_re.data.buffer, fk_re.data.byteOffset, nin), fkre_ptr / BYTES);
    view.set(new Float64Array(fk_im.data.buffer, fk_im.data.byteOffset, nin), fkim_ptr / BYTES);

    var ier = exports.nufft2d2(nj, x_ptr, y_ptr, cre_ptr, cim_ptr, iflag, tol, ms, mt, fkre_ptr, fkim_ptr);
    if (ier !== 0) {
      ...
      throw new RuntimeError("finufft2d2 failed with error code " + ier);
    }
