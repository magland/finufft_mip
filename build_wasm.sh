...
FINUFFT_SRC="$SCRIPT_DIR/finufft"
...
if [ ! -d "$FINUFFT_SRC" ]; then
  echo "Error: FINUFFT source not found at $FINUFFT_SRC" >&2
  echo "Make sure submodules are initialized: git submodule update --init" >&2
  exit 1
fi
...
# Find DUCC0 library (path varies by CMake version)
